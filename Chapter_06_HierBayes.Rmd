...
2. Update the data model to reference `Ex[i]` instead of `mu`
...
7.	Convert the random effects model to a hierarchical linear model by converting the mean, mu, to a linear model, `beta0 + beta1*y[i]` where y is the meteorological covariate you want to include, while keeping the random year effect.
